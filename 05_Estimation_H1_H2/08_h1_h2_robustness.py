"""Helpers for the final H1/H2 robustness pass.

This module keeps the robustness layer intentionally thin. The 
benchmark H1/H2 workflow already exists, so the p=3 pass should only 
do three things:

1. Reuse the same H1/H2 estimation code with one extra lag.
2. Save the outputs into a separate folder so the benchmark files stay 
   clean.
3. Build a compact comparison table that can be read alongside the 
   benchmark.
"""
...
ROBUSTNESS_P_LAGS = 3
ROBUSTNESS_OUTPUT_DIR = _config.ESTIMATION_DIR / "output" / "robustness_p3"
...
def save_family_outputs(family_result: dict[str, object]) -> None:
    ...
    pre_result.coefficient_table.to_csv(
        ROBUSTNESS_OUTPUT_DIR / f"h1h2_p3_{family}_pre_coefficients.csv",
        index=False,
    )
    post_result.coefficient_table.to_csv(
        ROBUSTNESS_OUTPUT_DIR / f"h1h2_p3_{family}_post_coefficients.csv",
        index=False,
    )
    ...
    for shock_output in family_result["shock_outputs"]:
        ...
        shock_output["pre_bands"].to_csv(
            ROBUSTNESS_OUTPUT_DIR / f"h1h2_p3_{family}_{shock_name}_pre_bands.csv",
            index=False,
        )
        shock_output["post_bands"].to_csv(
            ROBUSTNESS_OUTPUT_DIR / f"h1h2_p3_{family}_{shock_name}_post_bands.csv",
            index=False,
        )
        shock_output["difference_bands"].to_csv(
            ROBUSTNESS_OUTPUT_DIR / f"h1h2_p3_{family}_{shock_name}_post_minus_pre_bands.csv",
            index=False,
        )
        shock_output["pre_diagnostics"].to_csv(
            ROBUSTNESS_OUTPUT_DIR / f"h1h2_p3_{family}_{shock_name}_pre_draw_diagnostics.csv",
            index=False,
        )
        shock_output["post_diagnostics"].to_csv(
            ROBUSTNESS_OUTPUT_DIR / f"h1h2_p3_{family}_{shock_name}_post_draw_diagnostics.csv",
            index=False,
        )
...
def load_band_table(family: str, shock_name: str, regime: str) -> pd.DataFrame:
    """Load one saved p=3 band table."""

    path = ROBUSTNESS_OUTPUT_DIR / f"h1h2_p3_{family}_{shock_name}_{regime}_bands.csv"
...
def build_key_irf_summary_table() -> Path:
    ...
    path = ROBUSTNESS_OUTPUT_DIR / "h1h2_p3_key_irf_summary.csv"
    summary.to_csv(path, index=False)
    return path


def build_benchmark_comparison_table() -> Path:
    """Compare the benchmark p=2 key outputs with the p=3 robustness outputs."""

    benchmark = pd.read_csv(
        _config.ESTIMATION_DIR / "output" / "presentation" / "tables" / "h1h2_key_irf_summary.csv"
    )
    robustness = pd.read_csv(ROBUSTNESS_OUTPUT_DIR / "h1h2_p3_key_irf_summary.csv")
    ...
    for col in compare_cols:
        merged[f"{col}_delta_p3_minus_p2"] = merged[f"{col}_p3"] - merged[f"{col}_p2"]

    path = ROBUSTNESS_OUTPUT_DIR / "h1h2_p3_vs_p2_comparison.csv"
