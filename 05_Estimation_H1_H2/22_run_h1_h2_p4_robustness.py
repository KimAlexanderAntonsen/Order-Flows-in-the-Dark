"""Run the p=4 robustness pass for H1/H2."""
...
_robust = importlib.import_module("21_h1_h2_p4_robustness")
...
def main() -> None:
    """Run the H1/H2 robustness pass with p=4."""

    ensure_output_dir()
    universe = load_sp500_universe()

    print(
        "Running p=4 H1/H2 robustness on the canonical universe "
    ...
    summary.to_csv(ROBUSTNESS_OUTPUT_DIR / "h1h2_p4_run_summary.csv", index=False)

    key_summary_path = build_key_irf_summary_table()
    comparison_path = build_benchmark_comparison_table()

    print("\np=4 robustness summary")
    print(summary.to_string(index=False))
    print(f"\nSaved key-horizon summary to {key_summary_path}")
    print(f"Saved p=4 versus p=2 comparison to {comparison_path}")
